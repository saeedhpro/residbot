...
async def select_bank_type(update, context):
    ...
    keyboard = [
        ...
        [InlineKeyboardButton("بانک شهر", callback_data='shar')],
...
async def handle_source_account(update: Update, context):
    ...
    if context.user_data['bank_type'] == 'shar':
        await update.message.reply_text('شماره شبا را وارد کنید:')
        return GET_DEST_IBAN

    if context.user_data['bank_type'] == 'shar_paya':
        await update.message.reply_text('شماره شبا را وارد کنید:')
        return GET_DEST_IBAN

    if context.user_data['bank_type'] == 'shar_paya_2':
        await update.message.reply_text('شماره شبا را وارد کنید:')
        return GET_DEST_IBAN
...
async def create_and_send_receipt(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if bank_type == 'saderat':
        ...
        html_content = {
            ...
            'receiver': convert_numbers_to_farsi(context.user_data['receiver']),
            'description2': convert_numbers_to_farsi(context.user_data['description2']),
            'amount_toman': format_amount(convert_numbers_to_farsi(str(int(amount) / 10))),
            'amount': format_amount(convert_numbers_to_farsi(context.user_data['amount'])),
            'datetime': convert_numbers_to_farsi(context.user_data['datetime']),
            'description': convert_numbers_to_farsi(context.user_data['description']),
        }
    ...
    if context.user_data['bank_type'] == 'tejarat':
        ...
        options['height'] = '1280'
        options['width'] = '996'
    elif context.user_data['bank_type'] == 'sedarat_2':
        options['height'] = '1280'
        options['width'] = '635'
    elif context.user_data['bank_type'] == 'shar':
        options['height'] = '1280'
        options['width'] = '591'
    elif context.user_data['bank_type'] == 'shar_paya':
        options['height'] = '1280'
        options['width'] = '623'
        ...
        options['width'] = '739'
    hti = Html2Image()
    hti.screenshot(html_str=rendered_html, css_str='', save_as='page.png', size=(int(options['width']), int(options['height'])))
